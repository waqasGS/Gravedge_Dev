...
namespace Invector.vCharacterController.vActions
{
    ...
    public class vFreeClimb : vMonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            {
                input = new Vector3(TP_Input.horizontalInput.GetAxis(), TP_Input.verticalInput.GetAxis());
                ClimbHandle();
                ClimbJumpHandle();
                ClimbUpHandle();
            }
            ...
            {
                input = Vector2.zero;
                TP_Input.cc.animator.SetFloat("InputHorizontal", 0);
                TP_Input.cc.animator.SetFloat("InputVertical", 0);
            }
        }
        ...
        protected virtual void ClimbHandle()
        {
            ...
            if (dragInfo.canGo && !inClimbEnter && Physics.SphereCast(handTargetPosition + transform.forward * -TP_Input.cc._capsuleCollider.radius * 0.5f, TP_Input.cc._capsuleCollider.radius * 0.5f, transform.forward, out hit, climbEnterMaxDistance, draggableWall))
            {
                ...
                if (enterExitInput.GetButtonDown() && dragInfo.inDrag && input.magnitude == 0 && Time.time > (oldInput + 0.5f))
                    ExitClimb();
                else if (dragInfo.canGo && (enterExitInput.GetButton() || TP_Input.cc.input.y > 0.1f) && !dragInfo.inDrag && Time.time > (oldInput + 2f))
                    EnterClimb();
            }
            ClimbMovement();
        }
        ...
        protected virtual void ClimbMovement()
        {
            ...
            if (canMoveClimb)
            {
                TP_Input.cc.animator.SetFloat("InputHorizontal", horizontal, 0.2f, Time.deltaTime);
                TP_Input.cc.animator.SetFloat("InputVertical", vertical, 0.2f, Time.deltaTime);
            }
            else if (!inAlingClimb && !inClimbJump)
            {
                TP_Input.cc.animator.SetFloat("InputHorizontal", 0, 0.2f, Time.deltaTime);
                TP_Input.cc.animator.SetFloat("InputVertical", 0, 0.2f, Time.deltaTime);
            }

            if (input.y < 0 && Physics.Raycast(transform.position + transform.up * (TP_Input.cc._capsuleCollider.height * 0.5f), -transform.up, TP_Input.cc._capsuleCollider.height, TP_Input.cc.groundLayer))
            {
                ExitClimb();
            }
        }
                            ...
                            if (Physics.Raycast(hit.point + Vector3.up * -0.05f, Vector3.up, out hit, TP_Input.cc._capsuleCollider.height, obstacle))
                            {
                                if (hit.distance > TP_Input.cc._capsuleCollider.height * 0.5f)
                                {
                                    if (hit.distance < TP_Input.cc._capsuleCollider.height)
                                    {
                                        ...
                                        TP_Input.cc.animator.SetBool("IsCrouching", true);
                                    }
                                    ClimbUp();
                                }
                                else
        ...
        IEnumerator AlignClimb()
        {
            ...
            TP_Input.cc.animator.SetFloat("InputVertical", 1f);
        ...
        {
            inClimbJump = true;
            TP_Input.cc.animator.SetFloat("InputHorizontal", input.x);
            TP_Input.cc.animator.SetFloat("InputVertical", input.y);
        ...
        {            
            ...
            TP_Input.cc.animator.SetBool("IsGrounded", true);
        ...
        protected virtual void ExitClimb()
        {
            ...
            else
            {
                ...
                TP_Input.cc.animator.SetFloat("GroundDistance", 0);
            }
